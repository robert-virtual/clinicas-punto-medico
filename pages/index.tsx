...
const SHEETS_BASE_URL = "https://sheets.googleapis.com/v4/spreadsheets/1N3ybCqwq9xPiya0W9F8xWdMBOeGJyiz1sj15PMXkM2I";
...
const Home: NextPage = () => {
  ...
  const { data:count,error:countErr} = useSWR<IGetValuesRes>(getDermatologaCount,fetcher)

const [dermatologaEp,setDermatologaEp] = useState("")
  useEffect(()=>{
      if (count) {
       setDermatologaEp(`${SHEETS_BASE_URL}/values/dermatologa!A${Number(count.values[0][0]) - 10}:D${count.values[0][0]}?key=${access_token}`) 
      }
  ...
  return (
    <div className={styles.container}>
      <Head>
       ...
       <p>Dermatologa Pasientes: { countErr?"no se pudo cargar": !count?"cargando...":count.values[0][0]}</p>
        <div className={styles.grid}>
          ...
          {/* </a> */}
          {
            ...
            <div key={i.toString()} className={styles.card} >
              <h2>#{i+1} {e[0].split(" ").slice(0,2).join(" ")}</h2>
